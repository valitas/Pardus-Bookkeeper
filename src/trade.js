...
function configure() {
    ...
    if (!configured) {
        ...
        script.src = chrome.runtime.getURL("inject/buildingtrade.js");
...
function addKeyPress(data) {
    ...
    if (
        !Options ||
        !Options["enableAutoKey"] ||
        document
            .getElementsByTagName("h1")[0]
            .firstElementChild.src.indexOf("Trading Outpost") !== -1
    )
...
function onBuildingData(data) {
    ...
    } else addButton("Track", onTrackButtonClick);
}
...
function findTransferButton() {
    ...
    for (i = 0, end = inputs.length; i < end; i++) {
        input = inputs[i];
        if (input.type === "submit" && input.value === "<- Transfer ->")
            return input;
    }
    return null;
}
...
function onTrackButtonClick() {
    ...
    if (this.value == "Track") {
        ...
        chrome.storage.sync.get(
            [universe.key, buildingKey],
            onTrackingBuilding,
        );
    } else Building.removeStorage(userloc, universe.key, onBuildingUntracked);
...
function onBuildingTracked() {
    ...
    if (chrome.runtime.lastError)
        alert(
            "Cannot track more buildings\n" + chrome.runtime.lastError.message,
        );
    else btn.value = "Untrack";
}
...
function parsePage() {
    ...
    typeName = document
        .evaluate("//h1", document, null, XPathResult.STRING_TYPE, null)
        .stringValue.trim();
    ...
    s = document
        .evaluate(
            "./table/tbody/tr[position()=1]/td[position()=3]",
            document.forms.building_trade,
            null,
    ...
    while ((tr = trs.iterateNext()) !== null) parseRow(tr);
    ...
    freeSpace = parseInt(
        freeSpace.nextElementSibling.textContent.replace(/t|,/g, ""),
    );
    ...
    credits = parseInt(
        credits.nextElementSibling.textContent.replace(/,/g, ""),
    );
...
function updateBuilding(storeItems, building, callback) {
    ...
    if (Building.getTypeShortName(building.typeId) === "TO") {
        ...
        for (var key in pageData.buyAtPrices) {
            ...
            building.buying[id] =
                pageData.max[key] - pageData.amount[key] < 0
                    ? 0
                    : pageData.max[key] - pageData.amount[key];
            building.selling[id] =
                pageData.amount[key] - pageData.min[key] < 0
                    ? 0
                    : pageData.amount[key] - pageData.min[key];
        }
    }
    ...
    if (
        ...
        if (building.typeId == 17) {
            // MO. We basically redo our upkeep/ticksleft based on the entered grid strength, 0 at default.
            if (!document.getElementById("bookkeeper-grid"))
                buildMODOM(building);

            building.level = parseInt(
                document.getElementById("bookkeeper-grid").value,
            );
            ...
            let upkeepMO = {
                ...
                19: Math.max(
                    Math.max(0, building.level - 10) +
                        Math.max(0, building.level - 15),
                ),
            };
            ...
            cidlist.forEach(function (cid) {
                ...
                if (upklist[cid] > 0)
                    ticksLeft.push(
                        Math.floor(pageData.amount[cid] / upkeepMO[cid]),
                    );
            });
...
function overflowGuard() {
    ...
    pageData.upkeep.length < pageData.production.length
        ? (L = pageData.production.length)
        : (L = pageData.upkeep.length);
    for (var i = 0; i < L; i++) {
        if (pageData.upkeep[i]) perTick -= pageData.upkeep[i];
        if (pageData.production[i]) perTick += pageData.production[i];
    }
    ...
    if (pageData.freeSpace < pageData.ticksLeft * perTick) {
        ...
        div.textContent =
            "Overflows in " +
            (Math.floor(pageData.freeSpace / perTick) + 1) +
            " ticks.";
