...
(function () {
    ...
    function updateTicksToggle() {
        if (ticksEnabled) ticksToggle.classList.add("on");
        else ticksToggle.classList.remove("on");
    }
    ...
    function onMessage(event) {
        ...
        if (data.pardus_bookkeeper !== 2) return;
        ...
        if (element) sector = element.textContent.trim();
    ...
    function addTickThingies(cached) {
        ...
        if (cached.ticks === 0) elt.classList.add("red");
        else if (cached.ticks === 1) elt.classList.add("yellow");
        if (cached.stocked) {
