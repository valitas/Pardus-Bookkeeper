...
function migrateV19Storage() {
    ...
    function onV21Stored() {
        ...
        } else console.log("Migrated to V2.1!");
    }
    ...
    function migrateV19Building(loc, a) {
        ...
        level = parseInt(a[5]);
        if (isNaN(level) || level < 1) level = undefined;
        ...
        if (
            ...
            for (id in res_production) {
                if (amount[id] !== undefined && amount_min[id] !== undefined)
                    selling.push(id, amount[id] - amount_min[id]);
            }
            ...
            for (id in res_upkeep) {
                if (amount[id] !== undefined && amount_max[id] !== undefined)
                    buying.push(id, amount_max[id] - amount[id]);
            }

            if (selling.length === 0) selling = undefined;
            if (buying.length === 0) buying = undefined;
        }
    ...
    function numberOfTicks(level, res_upkeep, amount) {
        ...
        for (key in res_upkeep) {
            base = res_upkeep[key];
            tickAmount = Math.round(base * (1 + 0.4 * (level - 1)));
            ticks = Math.floor(amount[key] / tickAmount);
            if (ticks < lowest) lowest = ticks;
        }
...
function onMessage(message, sender, sendResponse) {
    ...
    if (handler) return handler(message, sendResponse, sender);
    return false;
}
...
OpHandlers.queryTicksLeft = function (message, sendResponse) {
    ...
    keys = message.ids.map(function (id) {
        return message.ukey + id;
    });
    ...
    function onData(data) {
        ...
        for (key in data) {
            ...
            r[building.loc] = {
                t: ticksNow,
                f: ticksNow === building.ticksLeft && building.isFullyStocked(),
...
var Building = (function () {
    ...
    Building.CATALOGUE = [
        ...
        { n: "Trading Outpost", s: "TO", i: "trade_outpost", bu: {}, bp: {} },
    ];
    ...
    Building.getTypeId = function (name) {
        if (NAME_IDS === undefined) {
            NAME_IDS = Building.CATALOGUE.reduce(function (name_ids, data, id) {
                name_ids[data.n] = id;
                return name_ids;
            }, {});
        }
    ...
    Building.getTypeIdByIcon = function (icon) {
        if (ICON_IDS === undefined) {
            ICON_IDS = Building.CATALOGUE.reduce(function (icon_ids, data, id) {
                icon_ids[data.i] = id;
                return icon_ids;
            }, {});
        }
    ...
    Building.prototype.getUpkeep = function () {
        if (this.cachedUpkeep) return this.cachedUpkeep;
        if (this.upkeep.length > 0) this.cachedUpkeep = this.upkeep;
        else if (this.level !== undefined)
            this.cachedUpkeep = this.getNormalUpkeep();
        else this.cachedUpkeep = [];
        return this.cachedUpkeep;
    };

    Building.prototype.getProduction = function () {
        if (this.cachedProduction) return this.cachedProduction;
        if (this.production.length > 0) this.cachedProduction = this.production;
        else if (this.level !== undefined)
            this.cachedProduction = this.getNormalProduction();
        else this.cachedProduction = [];
    ...
    Building.prototype.setOwner = function (owner) {
        if (owner !== undefined) {
            owner = String(owner);
            if (owner.length === 0) owner = undefined;
        }
        this.owner = owner;
    };

    Building.prototype.setSelling = function (a) {
        this.selling = a || [];
    };
    Building.prototype.setBuying = function (a) {
        this.buying = a || [];
    };
    Building.prototype.setMinimum = function (a) {
        this.minimum = a || [];
    };
    Building.prototype.setMaximum = function (a) {
        this.maximum = a || [];
    };
    Building.prototype.setPSB = function (a) {
        this.psb = a || false;
    };
    Building.prototype.setCredits = function (a) {
        this.credits = a || null;
    };
    ...
    Building.prototype.getCommoditiesInUse = function () {
        ...
        if (this.buying) this.buying.forEach(pushc);
        if (this.selling) this.selling.forEach(pushc);
    ...
    Building.prototype.hasProduction = function () {
        ...
        for (var i = 1; i < s.length; i++) {
            if (Building.isProduction(this.typeId, i.toString())) sum += s[i];
        }
        return sum > 0;
    };
    ...
    function unpackArray(a) {
        var r, i, end;
        if (!a) return [];
        for (r = [], i = 0, end = a.length; i < end; i += 2) r[a[i]] = a[i + 1];
        return r;
    }
    ...
    function computeUpPr(spec, prop, level, factor) {
        var base, k, r;
        if (spec === undefined || level === undefined) return undefined;
        base = spec[prop];
        r = [];
        for (k in base) r[k] = Math.round(base[k] * (1 + factor * (level - 1)));
        return r;
    }
...
var Sector = (function () {
    ...
    Sector.getId = function (sectorName) {
        if (NAME_IDS === undefined) {
            NAME_IDS = CATALOGUE.reduce(function (name_ids, data, id) {
                name_ids[data.n] = id;
                return name_ids;
            }, {});
        }
